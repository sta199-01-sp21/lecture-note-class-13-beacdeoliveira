...
When you use the interval you're more likely to capture the true value. There is a trade off between accuracy and perfection.
...
I would be surprised if 40 of them were left handed.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/Lecture Notes 13/manhattan.csv")
```
...
rnorm = will give a normal distribition
...
Not everyone got the same mean value. There's small variability between each run.
...
In repeated samples we're 95% confident that  the interval between 2302 and 2974 includes the true value.
...
We are 95% sure that the median will falls between 2162 and 2875 in terms of home rental price.
...
Pop = entire universe of cases

- Population: Mice not on Treatment
- Parameter of interest: The proportion
...
It means that we are 95% sure that the true proportion is within .690 and .797.
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_propy, level = .95)
ci_mice_lived
```
...
We are 95% sure that the prop will lie between .207 and .310.
